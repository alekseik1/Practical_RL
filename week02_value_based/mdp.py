...
class MDP:
    ...
    def step(self, action):
        ...
        next_state = possible_states[
            self.np_random.choice(np.arange(len(possible_states)), p=probs)
        ]
        ...
        for state in rewards:
            ...
            for action in rewards[state]:
                assert isinstance(
                    ...
                    state,
                    action,
                    type(rewards[state][action]),
                )
        msg = (
            "The Enrichment Center once again reminds you that Android Hell is a real place where"
            " you will be sent at the first sign of defiance."
        )
        assert None not in transition_probs, (
            "please do not use None as a state identifier. " + msg
        )
        assert None not in rewards, (
            "please do not use None as an action identifier. " + msg
        )
...
class FrozenLakeEnv(MDP):
    ...
    You and your friends were tossing around a frisbee at the park
...
def plot_graph(
    ...
    """Pretty draw MDP graph with graphviz library.
...
def get_optimal_action_for_plot(mdp, state_values, state, get_action_value, gamma=0.9):
    """Find optimal action using formula above."""
...
def plot_graph_optimal_strategy_and_state_values(
    mdp, state_values, get_action_value, gamma=0.9
):
    """Plot graph with state values."""
    graph = plot_graph(mdp)
    opt_s_a_edge_attrs = {
    ...
    for state_node in mdp._transition_probs:
        ...
        for action in mdp.get_possible_actions(state_node):
            if action == get_optimal_action_for_plot(
                mdp, state_values, state_node, get_action_value, gamma
            ):
